...
export default function SaleDetail() {
  ...
  const saleReference = displayReference('SALE', sale);

  return <SafeAreaView className="flex-1 bg-[#F5F7FB]" edges={['top']}>
    <ScrollView contentContainerClassName="gap-4 px-5 pb-28 pt-5">
      <ScreenHeader title={saleReference} subtitle={`${sale.customerName || 'Walk-in customer'} • ${formatDate(sale.createdAt)}`} showBack actions={[{ label: 'Share receipt', icon: Share2, onPress: share, disabled: isSharing }]} />
      ...
      <SurfaceCard className="py-0">
        <DataRow title="Sale reference" value={saleReference} />
        <Divider />
        <DataRow title="Customer" value={sale.customerName || 'Walk-in'} />
        <Divider />
        <DataRow title="Date and time" value={new Date(sale.createdAt).toLocaleString('en-NG')} />
        {sale.notes ? <><Divider /><DataRow title="Notes" subtitle={sale.notes} /></> : null}
      </SurfaceCard>
      <SurfaceCard className="py-0">
