...
export interface FeatureField { label: string; placeholder: string; type?: 'text' | 'date'; keyboardType?: 'default' | 'email-address' | 'numeric' | 'phone-pad'; secure?: boolean }
...
export default function FeaturePage({ config }: { config: FeatureConfig }) {
  ...
  const [values, setValues] = useState<Record<string, string>>(() => Object.fromEntries(
    (config.fields ?? []).filter((field) => field.type === 'date').map((field) => [field.label, todayKey()]),
  ));
  const submit = () => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F5F7FB]" edges={['top']}>
      <ScrollView contentContainerClassName="gap-4 px-5 pb-28 pt-5" keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        {config.banner ? <View className="rounded-[5px] border p-4" style={{ backgroundColor: bannerTone[0], borderColor: bannerTone[1] }}><Text className="text-[10px] font-bold" style={{ color: bannerTone[2] }}>{config.banner.label}</Text><Text className="mt-2 text-[13px] leading-[18px] text-[#0F172A]">{config.banner.text}</Text></View> : null}
        {config.fields?.length ? <View className="gap-4">{config.fields.map((field) => field.type === 'date'
          ? <DatePickerField key={field.label} label={field.label} value={values[field.label] ?? todayKey()} onChange={(value) => setValues((current) => ({ ...current, [field.label]: value }))} />
          : <Input key={field.label} label={field.label} placeholder={field.placeholder} value={values[field.label] ?? ''} onChangeText={(value) => setValues((current) => ({ ...current, [field.label]: value }))} keyboardType={field.keyboardType} isPassword={field.secure} />)}</View> : null}
        ...
        {config.primaryAction ? <Button title={config.primaryAction} onPress={submit} className="h-14 rounded-[5px]" /> : null}
        {config.secondaryAction ? <Button title={config.secondaryAction} variant="secondary" onPress={() => config.secondaryRoute && router.push(config.secondaryRoute as never)} className="h-14 rounded-[5px]" /> : null}
      </ScrollView>
    </SafeAreaView>
  );
}
